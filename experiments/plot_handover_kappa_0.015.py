...
NPZ_FILES = {
    # DDPP와 MaxSNR만 기존 summary NPZ에서 읽습니다.
    "DDPP": (
...
RAW_KAPPA = 0.015
RAW_TRAIN_SEEDS = (0, 1, 2)
RAW_EVAL_SEEDS = (2000, 2001, 2002, 2003, 2004)
...
def load_raw_handover_trajectory(
    algorithm_name,
    result_root,
    folder_prefix,
    kappa,
    ...
    """
    PF-HAPPO, Jensen-HAPPO, HeLyMARL의 raw evaluation 파일을
    동일한 방식으로 집계합니다.

    집계 순서:
        1) 동일 train seed의 eval seeds를 먼저 평균
        2) train-seed 평균 및 표준편차 계산

    folder_prefix가 None이면:
        result_root/kappa_0.015_seed_0/eval_seed_2000.npz

    folder_prefix가 "HAPPO"이면:
        result_root/HAPPO_kappa_0.015_seed_0/eval_seed_2000.npz
    """
    ...
    for train_seed in train_seeds:
        if folder_prefix:
            ...
            run_folder = f"kappa_{kappa_tag}_seed_{train_seed}"
        ...
        for eval_seed in eval_seeds:
            ...
            if not os.path.exists(eval_path):
                print(
                    f"[Warning] {algorithm_name} eval file not found: "
                    f"{eval_path}"
                )
                continue
        ...
        if len(eval_trajectories) == 0:
            print(
                f"[Warning] No valid {algorithm_name} eval files "
                f"for train seed {train_seed}: {run_dir}"
            )
            continue
        ...
        # 동일 train seed의 eval seed들을 먼저 평균
        ...
        print(
            f"[{algorithm_name} kappa={kappa_tag}] "
            f"train_seed={train_seed}, "
            f"eval_seeds={successful_eval_seeds}"
        )

    if len(trajectory_per_train_seed) == 0:
        prefix_text = f"{folder_prefix}_" if folder_prefix else ""
        raise RuntimeError(
            f"{algorithm_name} raw results를 찾지 못했습니다: "
            f"{result_root}/{prefix_text}kappa_{kappa_tag}_seed_*"
        )
    ...
    return calculate_seed_statistics(
        ...
        loaded_key=(
            f"raw {result_root}/{prefix_text}kappa_{kappa_tag}_seed_*/"
            f"eval_seed_*.npz; keys={sorted(loaded_keys)}; "
...
# PF-HAPPO / Jensen-HAPPO: results/results_baselines/{pf,jensen}/ raw eval
for algorithm, config in BASELINE_RAW_CONFIGS.items():
    try:
        result = load_raw_handover_trajectory(
            algorithm_name=algorithm,
            result_root=config["result_root"],
            folder_prefix=config["folder_prefix"],
            kappa=RAW_KAPPA,
            train_seeds=RAW_TRAIN_SEEDS,
            eval_seeds=RAW_EVAL_SEEDS,
            max_steps=MAX_STEPS,
            smooth_window=SMOOTH_WINDOW,
        )
        trajectory_results[algorithm] = result
        ...
        print(
            f"[{algorithm}] key={result['loaded_key']}, "
...
# HeLyMARL: results/results_kappa/HAPPO_kappa_... raw eval
try:
    result = load_raw_handover_trajectory(
        algorithm_name="HeLyMARL",
...
# 0.08 이하 구간만 0.01 간격으로 세로축 눈금 표시
# y축 상한 자체는 기존 자동 범위를 그대로 유지
Y_TICK_INTERVAL = 0.01
Y_TICK_MAX = min(0.08, y_upper)

ax.set_yticks(
    np.arange(
        0.0,
        Y_TICK_MAX + Y_TICK_INTERVAL / 2,
        Y_TICK_INTERVAL,
    )
)
...
png_path = os.path.join(
    SAVE_DIR,
    f"mean_cumulative_handover_ratio_"
    f"5algorithms_kappa{RAW_KAPPA:.3f}_3train_5eval.png",
)
